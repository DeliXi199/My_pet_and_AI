...
class DeskPet(QtWidgets.QLabel):
    ...
    def initUI(self):
        ...
        self.changeDirectionTimer.timeout.connect(
            self.changeDirection
        )  # 定时器触发时调用changeDirection方法
        self.startIdle()
    ...
    def loadImages(self, path):
        return [
            QtGui.QPixmap(os.path.join(path, f))
    ...
    def changeDirection(self):
        if (
            self.currentAction == self.startFall
            or self.currentAction == self.eating
            or self.currentAction == self.transform
            or self.currentAction == self.sleep
            or self.currentAction == self.pipi
            or self.currentAction == self.exercise
            or self.currentAction == self.WakeUp
            or self.currentAction == self.startIdle
            or self.startMeet
        ):
